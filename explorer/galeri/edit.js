/*
 * Tampilan form EDIT dokumen "galeri" — dipanggil oleh
 ...
 *   data          - hasil NxStorage('galeri') (skema production)
...
export async function render(pageContainer, data, row, ctx) {
  ...
  pageContainer.innerHTML = `
    <div style="padding: 24px; max-width: ${CONFIG.formMaxWidth}; margin: 0 auto;">
      <h2 style="margin: 0 0 8px 0; font-size: 22px; font-weight: 600; color: var(--beranda-dv-text);">${data.appname || 'galeri'} — Ubah</h2>
      <p style="margin: 0 0 20px 0; color: var(--beranda-dv-text-muted);">${data.deskripsi || ''}</p>
      <div id="nx-galeri-edit-form-host"></div>
    </div>
  `;

  const formHost = pageContainer.querySelector('#nx-galeri-edit-form-host');
