/*
 * Tampilan form dokumen "Posdip" — dipanggil oleh ../index.js (renderDocForView)
 * lewat import('./Posdip/form.js'), menerima `data` (hasil NxStorage('Posdip'))
 ...
 * upload), supaya bisa diubah bebas kapan saja KHUSUS untuk Posdip (validasi,
...
export async function render(container, data) {
    ...
    container.innerHTML = `
      <div style="padding: 24px; max-width: ${CONFIG.formMaxWidth}; margin: 0 auto;">
        ...
        <fluent-card style="padding: 16px;">
          <p style="margin: 0; font-size: 13px; color: var(--beranda-dv-text-muted);">Skema form (storage/Posdip.json → production) tidak ditemukan.</p>
        </fluent-card>
      </div>
    `;
    return;
  ...
  // di Posdip.json namanya "file", bukan "file_path" seperti mscp/ipkd. Versi awal
  // file ini salinan mscp dan masih memakai "file_path", sehingga
  ...
  const generatedForm = await mountGeneratedForm(formHost, data, {
    onSubmit: (values) => {
      ...
      if (!uploadField) {
        Notif.show({ type: 'error', title: 'Skema tidak punya field upload', subtitle: 'Tidak ada field bertipe "file" di Posdip.json' });
        return;
      }

      // Field upload dipisahkan dari values — sisanya ikut sebagai kolom
      // tambahan via `field`.
      ...
      NXUI.Storage().cloud().addBackground({
        tabel: data.production.tabel || 'Posdip',
        file: uploadValue,
    ...
    onReset: () => {
      console.log('[explorer/Posdip] form direset');
    },
  });
}
