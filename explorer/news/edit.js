/*
 * Tampilan form EDIT dokumen "news" (Postingan) — dipanggil oleh
 ...
 * bawahnya dibatasi 960px — dua elemen dengan lebar beda dalam satu tampilan,
 * terlihat tidak menyatu. Dengan render ulang penuh di sini, mode edit
 * BENAR-BENAR independen secara visual dari tabel — tidak bergantung pada
 * markup apa pun yang sudah ada di tabel.js.
 *
 ...
 *   data          - hasil NxStorage('news') (skema production)
 *   row           - baris yang sedang diedit (nilai lama, dari server)
 *   ctx           - helper dari tableBuilder.js:
 *     ctx.fileFieldNames        Set<string> nama field type:"file" (mis. "images")
...
export async function render(pageContainer, data, row, ctx) {
  ...
  pageContainer.innerHTML = `
    <div style="padding: 24px; max-width: ${CONFIG.formMaxWidth}; margin: 0 auto;">
      <h2 style="margin: 0 0 8px 0; font-size: 22px; font-weight: 600; color: var(--beranda-dv-text);">${data.appname || 'Postingan'} — Ubah</h2>
      <p style="margin: 0 0 20px 0; color: var(--beranda-dv-text-muted);">${data.deskripsi || ''}</p>
      <div id="nx-news-edit-form-host"></div>
    </div>
  `;

  const formHost = pageContainer.querySelector('#nx-news-edit-form-host');
  ...
  await mountGeneratedForm(formHost, data, {
    ...
    onSubmit: async (values) => {
      try {
        ...
        ctx.notifySuccess('Postingan berhasil diubah');
